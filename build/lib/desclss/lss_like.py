...
class LSSLikelihood(object):
    def __init__(self,saccin, t1=None, t2=None, zmin=None, zmax=None, lmin=None, lmax=None, diagonal=False) :
        if (type(saccin)==type("filename")):
            self.s=sacc.SACC.loadFromHDF(saccin, zmin=zmin, zmax=zmax, lmin=lmin, lmax=lmax, diagonal=diagonal)
        else:
            self.s=saccin
